...
#
# Блок 1 — исторические ключи. Значений «Отгружен»/«Доставлен» в УТ 11 нет,
# но ключи сохраняются для обратной совместимости с ранее импортированными
# данными и внешними системами.
#
# Блок 2 — реальные статусы УТ 11 («Состояние» документа «Заказ клиента»).
# Без них «Закрыт» отбрасывался как unknown ещё до записи в БД, из-за чего
# заказ никогда не переходил в `delivered` и бонусы тренерам не начислялись.
STATUS_MAPPING: dict[str, str] = {
    # Исторические ключи (обратная совместимость)
