...
#    E2E Tests возвращён в список 2026-09-20: чек стабильно зелёный (четыре прогона
#    подряд, 2m38s-3m10s) и втрое короче build (3.12), поэтому ожидание PR не растёт.
#    Вместе с возвратом с его pull_request снят paths-фильтр — иначе см. пункт 4.
#    Фильтрация переехала внутрь job'а: шаг «Нужен ли прогон» смотрит diff через
#    gh pr diff, и без правок во frontend/** тяжёлые шаги пропускаются (~20 секунд
#    на такой PR), но статус контекста приходит всегда.
