...
**Frontend-роутинг CMS-страниц:**

`/(blue)/[slug]/page.tsx` — универсальный динамический маршрут: фетчит `/api/v1/pages/{slug}/`, отдаёт 404 если страница не найдена или не опубликована. Любая страница, созданная в Django Admin с `is_published=True`, автоматически доступна по своему slug. Статические маршруты (например `/privacy-policy`) имеют приоритет над `[slug]` и используют собственный `page.tsx`.

**Известные slug:**

| slug | Страница | Frontend-маршрут | Примечание |
|------|----------|-----------------|------------|
| `privacy-policy` | Политика обработки персональных данных | `/privacy-policy` | Собственный `page.tsx`, ISR 1ч |

> **Не через CMS:** `/requisites` реализован как статическая страница (`(blue)/requisites/page.tsx`) с захардкоженными константами. В дальнейшем — отдельная модель `Company` с структурированными полями, не общий Pages CMS.
