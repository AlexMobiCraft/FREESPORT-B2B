...
**Маркеры `unit` / `integration` / `performance` проставляются автоматически по каталогу теста** — руками писать их не нужно. Разметку делает хук `pytest_collection_modifyitems` в `backend/conftest.py`; авторитетное описание правила и таблица соответствия — `backend/docs/testing-standards.md`, раздел «Маркеры pytest».

- Категорию задаёт каталог: `unit/` → `unit`; `integration/`, `functional/`, `regression/` → `integration`; `performance/` → `performance`. Тесты в `apps/` без каталога-категории считаются `unit`.
- **Явный маркер в файле переопределяет автоматический.** Ставьте его, когда каталог классифицирует тест неверно.
- `@pytest.mark.django_db` - для всех тестов, использующих БД; автоматической разметке не мешает и ставится вручную.

> ⚠️ Маркер `unit` теперь означает «модульный тест приложения», а не «без БД». Тест с `django_db` внутри `apps/` штатно получает `unit` — это ожидаемо. Прежнее правило «не маркировать тесты с БД как `unit`» больше не действует.
