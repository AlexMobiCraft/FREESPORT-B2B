...
### 1.1. Архитектурный обзор ([подробности](../integrations/1c/architecture.md))
...
> **Статус: Планируется.** Интеграция с ЮKassa не реализована. В модели заказа есть placeholder-поля (`payment_id`, `payment_method`, `payment_status`), но клиентского кода, webhook-обработчиков и API-эндпоинтов для оплаты нет. Статусы оплаты управляются вручную.
...
> **Статус: Планируется.** Интеграция с СДЭК и Boxberry не реализована. Существует базовая модель `DeliveryMethod` со статическими методами доставки (курьер, самовывоз, транспортная компания), управляемая через Django Admin. Расчёт стоимости, трекинг и API внешних служб не подключены.
...
Для полной информации об архитектуре интеграции с 1С см. [Архитектура интеграции 1С](../integrations/1c/architecture.md), включающая:
