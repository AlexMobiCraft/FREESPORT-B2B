...
7. **Загрузка статусов сайта в таблицу сопоставления 1С-Битрикс:**
   - Подтвердите, что кнопка **"Загрузить с сайта"** в настройках **"Статусы заказов"** выполняет запрос `type=sale&mode=info` к URL обмена.
   - Какие XML-теги ожидает обработчик 1С для справочника статусов: `Справочник`, `Cтатусы`, `Элемент`, `Ид`, `Название`, `ПлатежныеСистемы`?
   - Какое значение 1С использует после сопоставления при обратной выгрузке статуса на сайт: `Ид`, `Название`, внутренний статус 1С или результат сопоставления?
   - Нужно ли включать в справочник сайта начальный статус `Не согласован`, если сайт отправляет его в реквизите `Статус заказа` при первичной выгрузке заказа?
   - В каком реквизите 1С будет отправлять статус обратно: `СтатусЗаказа`, `Статус Заказа`, `Статус заказа` или другом поле?

### 3.4 **ВОПРОСЫ ПО СИНХРОНИЗАЦИИ ПОКУПАТЕЛЕЙ:**

8. **Загрузка существующих клиентов из 1С:**
...
10. **Настройки webhook для нашей платформы:**
