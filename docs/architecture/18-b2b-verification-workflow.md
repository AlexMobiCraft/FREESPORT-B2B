...
В обоих случаях ответ `UserRegistrationView.post()` не содержит JWT и не раскрывает ФИО/роль найденной записи; `UserConsent` для этого пути не создаётся, так как это привязка, а не новая регистрация.
