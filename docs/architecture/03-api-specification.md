...
> [!IMPORTANT]
> Актуализация от `2026-05-02`: frontend и email/PDF paths отображают UI-формат номера (`CCCC-YYNNN` / `CCCCC-YYNNN-S`), но API хранит и отдаёт только канонический `order_number`. Создание заказа для гостя больше не поддерживается: доступ к checkout flow требует аутентификацию. `customer_code` присваивается автоматически при первом заказе, если у пользователя его ещё нет (см. `2026-07-11`).
