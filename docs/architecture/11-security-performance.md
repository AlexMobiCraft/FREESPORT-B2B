...
- `SubscribeForm` (Blue) и `ElectricSubscribeForm` (Electric) содержат **два** обязательных неотмеченных чек-бокса (стори 41.11, 2026-09-12): согласие на обработку ПДн (`pdp_consent`, со ссылкой на политику) и согласие на рассылку по электронной почте (`marketing_consent`). Тексты не пересекаются: ст. 9 ч. 1 152-ФЗ в ред. 156-ФЗ требует оформлять согласие на ПДн отдельно от иных документов, в том числе от согласия на рекламу по ст. 18 38-ФЗ. Объединённый чекбокс редакции 2 стори 41.3 отменён. Кнопка «Подписаться» неактивна, пока не отмечен ПДн; без галочки рассылки отправку останавливает валидация формы, ошибка и фокус — у этого чекбокса. Оба чекбокса объявлены обязательными через `aria-required="true"`, а не `required`: нативная валидация перехватила бы отправку до `react-hook-form`.
- При подписке backend создаёт **две** записи `UserConsent` (pdp_contract + marketing_email) с одинаковыми IP/User-Agent/given_at. У каждой своя версия формулировки: `pdp_contract` — поверхности реестра `newsletter_pdp_checkbox`, `marketing_email` — `newsletter_marketing_checkbox`. Поверхность `newsletter_checkbox` с объединённым текстом осталась в реестре без привязок: на её версию ссылаются записи журнала до выката 41.11, и они обязаны разрешаться в свой текст.
...
- `POST /api/v1/subscribe/` принимает обязательные `pdp_consent: true` и `marketing_consent: true` — строго JSON boolean `true`. `false`, `null`, `"true"`, `1`, `"on"` и отсутствие поля дают `400 Bad Request`; если не выставлены оба флага, обе ошибки приходят одним ответом, а не по одной.
...
- Соответствие текста форм и реестра стережёт Vitest-тест `frontend/src/__tests__/consent-texts-registry.test.tsx`: он рендерит четыре формы и сверяет доступное имя каждого из шести чекбоксов согласия (по два в каждой форме; в подписке — со стори 41.11) с текущей ревизией его поверхности в реестре. Тест обязан падать (не пропускаться) при отсутствии файла реестра — бэкенд-тесты сделать этого не могут, в тестовый контейнер смонтирован только `backend/`.
- **Клиент присылает показанную версию, сервер её сверяет (доработка по ревью, 2026-09-09).** `POST /api/v1/subscribe/` требует `pdp_consent_text_version` и `marketing_consent_text_version` — обе безусловно, потому что оба согласия подписки обязательны (стори 41.11; до неё — одно поле `consent_text_version` на объединённый чекбокс, запрос в прежнем формате получает `400 consent_text_outdated`). `POST /api/v1/auth/register/` требует `pdp_consent_text_version` и (при `marketing_consent: true`) `marketing_consent_text_version`. Имена полей версии у двух эндпоинтов общие, и каждое поле сверяется со своей привязкой реестра: версия ПДн, присланная в поле рассылки, отклоняется. Несовпадение с действующей ревизией или отсутствие поля — `400` с кодом `consent_text_outdated` и требованием обновить страницу. Текст требования один для любой ошибки поля версии — в том числе для массива, объекта, boolean, ноль-байта и одиночного суррогата: `CharField` отвечает на них собственными сообщениями, и ответ выравнивает `consent_text_outdated_payload()` (седьмой круг ревью, 2026-09-10). Версии сверяются field-level валидаторами сериализаторов, а не в object-level `validate()`, поэтому код приходит и тогда, когда в запросе есть другие ошибки (email, роль, галочка, несовпадение паролей) — DRF при любой field-level ошибке `validate()` не вызывает (пятый круг ревью, 2026-09-10). Значения приходят из константы `frontend/src/constants/consentTexts.ts`, собранной в тот же бандл, что и сам текст. Запрошенная у сервера версия не дала бы ничего — старая вкладка получила бы актуальное значение и записала согласие на формулировку, которой не видела. В журнал кладётся значение из реестра, а не присланное клиентом.
