...
1.  **Категории (`groups.xml`)**: Создание полного дерева категорий 1С. Корневой узел `СПОРТ` сохраняется в `Category` как внутренний якорь, а его прямые потомки становятся публичными root-узлами только на уровне API-проекции `/api/v1/categories-tree/`.
...
- **Полное дерево категорий 1С + публичная проекция**: `VariantImportProcessor` сохраняет цепочку `СПОРТ -> дочерняя категория -> потомок` в БД. Витрина читает не raw-root категории, а `/api/v1/categories-tree/`, где `СПОРТ` скрыт, а технические fallback/placeholder категории рекурсивно исключены.
- **Fallback для неизвестных category_id**: если товар из `goods.xml` ссылается на неизвестную категорию, импорт не создаёт публичный root вида `Категория <uuid>`. Такие товары изолируются в скрытой технической категории `onec-unresolved-category`, а счётчик фиксируется в статистике `category_fallbacks`.
