...
- [Команда fix_category_tree_public_roots](#команда-fix_category_tree_public_roots)
...
## Команда fix_category_tree_public_roots

### Описание

Восстанавливает данные после старого импорта категорий, где дети `СПОРТ` могли стать `parent=None`, а неизвестные `category_id` из `goods.xml` могли создать публичные корни-заглушки вида `Категория <uuid>`.

Актуальный контракт:

- `Category` хранит полное дерево 1С, включая root `СПОРТ`.
- `/api/v1/categories-tree/` отдаёт публичную проекцию: прямые дети `СПОРТ` как корни витрины.
- Служебные категории `uncategorized`, `onec-unresolved-category`, `Без категории` и узлы-заглушки не должны попадать в публичное дерево.

Команда не удаляет полезные товары каскадно: полезные корневые категории переносит под `СПОРТ`, товары из веток-заглушек переносит в скрытую fallback-категорию `onec-unresolved-category`, сами ветки-заглушки деактивирует.

### Синтаксис

```bash
python manage.py fix_category_tree_public_roots [--execute] [--root-name=СПОРТ]
```
...
| Параметр      | Тип    | По умолчанию | Описание                                                        |
| ------------- | ------ | ------------ | --------------------------------------------------------------- |
| `--execute`   | flag   | false        | Применить изменения в БД. Без флага выполняется только аудит.   |
| `--root-name` | string | `СПОРТ`      | Имя якорной категории, если отличается от `ROOT_CATEGORY_NAME`. |

### Рекомендуемый порядок

```bash
# 1. Аудит без изменений
python manage.py fix_category_tree_public_roots

# 2. Применение после проверки отчёта
python manage.py fix_category_tree_public_roots --execute
```

Dry-run выводит статус якоря, количество публичных корневых категорий, количество корней-заглушек и список затронутых товаров в ветках-заглушках. Если найдено несколько корневых `СПОРТ`, команда завершается ошибкой, чтобы оператор сначала разобрал дубли вручную.

### Важно про старую cleanup-команду

`cleanup_root_categories` относится к старому контракту, где дети `СПОРТ` становились `parent=None`, а сам `СПОРТ` удалялся. После перехода на публичную API-проекцию эту команду нельзя использовать как восстановление дерева витрины без изменения её семантики: она может удалить внутренний якорь `СПОРТ`, который теперь обязателен для корректного `/api/v1/categories-tree/`.
