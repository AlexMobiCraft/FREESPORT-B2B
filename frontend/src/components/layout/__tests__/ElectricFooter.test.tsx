...
describe('ElectricFooter', () => {
  ...
  it('сохраняет ссылки нижней панели', () => {
    ...
    expect(screen.getByRole('link', { name: 'Согласие на получение рекламы' })).toHaveAttribute(
      'href',
      '/marketing-consent'
    );
  ...
  it('ряд ссылок нижней панели переносится по строкам', () => {
    // Регрессия код-ревью: кнопка стала третьим элементом ряда (теперь их четыре). Без переноса
