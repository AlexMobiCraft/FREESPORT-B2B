...
describe('LoginForm', () => {
  ...
  describe('Rendering', () => {
    test('should render email and password fields', () => {
      ...
      expect(screen.getByLabelText(/учетное имя/i)).toBeInTheDocument();
    ...
    test('should have proper autocomplete attributes', () => {
      ...
      const emailInput = screen.getByLabelText(/учетное имя/i);
  ...
  describe('Story 41.12 — AC4/AC5/AC6: информирование и защита редиректа', () => {
    ...
    test.each([
      ...
      await user.type(screen.getByLabelText(/учетное имя/i), 'test@example.com');
  ...
  describe('Loading State', () => {
    ...
    test('should disable form inputs during submission', async () => {
      ...
      const emailInput = screen.getByLabelText(/учетное имя/i) as HTMLInputElement;
