...
describe('Реестр текстов согласий сверен с формами', () => {
  ...
  it('константы версий фронта совпадают с действующими ревизиями реестра', () => {
    ...
    // Стори 41.20 (решение D5): тексты обеих маркетинговых поверхностей совпадают
    // дословно, поэтому совпадают и их хеши — версии различает только метка ревизии.
    // Совпади версии целиком, загрузчик реестра отверг бы загрузку как дубль ревизии.
    expect(CONSENT_TEXT_VERSIONS.newsletterMarketing).not.toBe(
      CONSENT_TEXT_VERSIONS.registrationMarketing
    );
  });
  ...
  it('исторические версии остаются в known_versions', () => {
    ...
    // Стори 41.20 добавила: прежние формулировки рассылки подписки и регистрации
    // (стори 41.11) — на них ссылаются записи журнала прода.
